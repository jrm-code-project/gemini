...
(defun analyze-system-definition-with-session (session system-name)
    ...
    (let ((source-file (asdf:system-source-file asdf-system)))
      ...
      (let ((*system-instruction*
              (content
               ...
               :role "system")))
        (invoke-gemini-with-session session
         (list
...
(defun analyze-system-definition (system-name)
  (analyze-system-definition-with-session (ensure-runtime-session) system-name))
...
(defun analyze-component-with-session (session system-name component-name)
...
(defun analyze-component (system-name component-name)
  (analyze-component-with-session (ensure-runtime-session) system-name component-name))
...
(defun analyze-lisp-with-session (session form &key package)
  (declare (ignore package))
  (gemini::gemini-pro-with-session session
   (list
...
(defun analyze-lisp (form &key package)
  (analyze-lisp-with-session (ensure-runtime-session) form :package package))
...
(defun analyze-file-with-session (session filename &key (temperature 0.01) (verbose t))
  ...
  (let ((session* (ensure-runtime-session session))
    ...
    (unwind-protect
         (prog1 (let iter ((current-form (car forms))
                           (analysis (continue-gemini-with-session session*
                  ...
                  (if (null more)
                      ...
                      (iter (car more)
                           (continue-gemini-with-session session*
                             (list (part +analyze-file-form-prompt+)
                                   (part (car more))))
...
(defun analyze-file (filename &key (temperature 0.01) (verbose t))
  (analyze-file-with-session (ensure-runtime-session)
                             filename
                             :temperature temperature
                             :verbose verbose))
