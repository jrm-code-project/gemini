...
(defun invert-current-session-prior-context ()
  "Invert the active session's prior context."
  (let ((session (ensure-runtime-session)))
    (setf (runtime-session-prior-context session)
          (invert-context (runtime-session-prior-context session)))
    session))
  ...
  (if (= n-steps step)
      ...
      (conversation
       ...
       (if (zerop step)
           ...
           (progn
             (invert-current-session-prior-context)
