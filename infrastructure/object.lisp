...
(defclass persona-config ()
  ((name :initarg :name :accessor get-name)
   ...
   (enable-diary-tools   :initarg :enable-diary-tools   :initform nil :accessor get-enable-diary-tools)
   ...
   (enable-lmstudio-tools :initarg :enable-lmstudio-tools :initform nil :accessor get-enable-lmstudio-tools)
   (enable-mcp-tools     :initarg :enable-mcp-tools     :initform t :accessor get-enable-mcp-tools)
...
(defclass openai-backend (backend)
  ...
  (:documentation "Concrete backend implementation for OpenAI-compatible chat-completions APIs."))

(defclass lmstudio-backend (backend)
  ((url :initarg :url :accessor get-backend-url :initform nil))
  (:documentation "Concrete backend implementation for LM Studio's /api/v1/chat streaming API."))
...
(defun resolve-backend-instance (config)
  (let ((api (get-googleapi config)))
    (cond ((eq api :google-interactions-api)
          ...
          ((eq api :lmstudio-api)
           (make-instance 'lmstudio-backend :url (get-url config)))
          ((eq api :openai-api)
...
(defclass content-generator ()
  ((config :initarg :config :accessor get-config)
   (memory-mcp-server :initform nil :reader get-memory-mcp-server)
...
(defmethod get-memory-mcp-server :around ((object content-generator))
  (or (call-next-method)
      (let ((config (get-config object)))
        (when (and config
                   (get-enable-mcp-tools config))
          (let ((server (memory-mcp-server (persona-memory-file config))))
            (when server
              (setf (slot-value object 'memory-mcp-server) server))
            server)))))
...
(defmethod get-enable-diary-tools ((object content-generator))
  (get-enable-diary-tools (get-config object)))
...
(defmethod get-enable-lmstudio-tools ((object content-generator))
  (get-enable-lmstudio-tools (get-config object)))
