...
(defun log-threat-safely (condition)
  (log-warn "Predator threat eliminated: ~A" condition))

(defun safely-close-predator-stream (stream)
  "Closes a Predator-supervised stream and logs teardown failures."
  (handler-case
      (close stream :abort t)
    (error (e)
      (log-warn "Predator supervisor failed while closing a stream: ~A" e))))

(defun sever-socket-connection (stream)
  (when (streamp stream)
    (safely-close-predator-stream stream)))
