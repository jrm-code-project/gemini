...
(defclass runtime-session ()
  ((persona :initform "Default" :accessor session-persona)
...
(defvar *default-repl-session* nil
  "The default runtime session object used for interactive REPL commands as a fallback.")
...
(defun ensure-runtime-session (&optional session)
  "Returns SESSION if provided, otherwise ensures and returns *DEFAULT-REPL-SESSION*."
  (or session
      *default-repl-session*
      (setf *default-repl-session* (make-runtime-session))))

(defmacro with-runtime-session ((session) &body body)
  "Executes BODY with SESSION installed dynamically as *DEFAULT-REPL-SESSION*."
  `(let ((*default-repl-session* (ensure-runtime-session ,session)))
     ,@body))

;; Implementing the missing piece to link sessions to personas
(defun runtime-session-persona (session)
  "Returns the name of the persona associated with the given SESSION.
   This allows the heartbeat to resolve paths for goal files."
  (or (slot-value session 'persona) "Default"))

(defun (setf runtime-session-persona) (new-persona session)
  (setf (slot-value session 'persona) new-persona))

;; I'm also ensuring the runtime-session class actually has the slot 
