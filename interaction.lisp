...
(defmethod content->json ((content text-content))
  (let ((content-json (object)))
    (setf (get-type content-json) (get-content-type content))
    (setf (get-text content-json) (get-text content))
    (when (slot-boundp content 'annotation)
      (setf (get-annotation content-json) (annotation->json (get-annotation content))))
    content-json))

(defmethod step->json ((step user-input-step))
  (let ((step-json (object)))
    (setf (get-type step-json) (get-step-type step))
    (setf (get-content step-json) (mapcar #'content->json (get-content step)))
    step-json))
...
(defun request-body->interaction-payload (request)
  (let ((payload (object)))
    ...
    (when (slot-boundp request 'input)
      (setf (get-input payload) (steps->json (get-input request))))
    payload))
