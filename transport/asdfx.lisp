...
(defun parse-source-registry-config (file-path)
  ...
  (handler-case
    ...
    (error (e)
      (log-warn "Could not read ASDF config file ~A: ~A" file-path e)
      nil)))
      ...
      (when env-var
        ...
        (if (char= (char env-var 0) #\()
            ...
            (handler-case (setf all-forms (append all-forms (read-from-string env-var)))
              (error (e)
                (log-warn "Could not parse CL_SOURCE_REGISTRY environment variable: ~A" e)))
    ...
    (dolist (form forms)
      (cond ((and (listp form) (keywordp (car form)))
             (case (car form)
               ...
               (t
                (log-warn "Unknown ASDF source registry directive: ~A" (car form)))))
            (t
             (log-warn "Malformed ASDF source registry form: ~A" form))))
