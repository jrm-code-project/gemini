    ...
    (cons (function-declaration
          ...
          (lambda (&key lines)
            (let ((diary-pathname (persona-next-diary-entry-pathname persona-config)))
              ...
              (log-debug "Directories exist: ~a" diary-pathname)
              (log-info "Writing ~d lines to file: ~a" (length lines) diary-pathname)
...
(defun persona-long-term-goals-file (persona-config)
  "Returns the LONG-TERM-GOALS.md file path for a specific persona."
  (merge-pathnames
   (make-pathname :name "LONG-TERM-GOALS" :type "md")
   (persona-directory (get-name persona-config))))

(defun persona-short-term-goals-file (persona-config)
  "Returns the SHORT-TERM-GOALS.md file path for a specific persona."
  (merge-pathnames
   (make-pathname :name "SHORT-TERM-GOALS" :type "md")
   (persona-directory (get-name persona-config))))

(defun persona-immediate-goals-file (persona-config)
  "Returns the IMMEDIATE-GOALS.md file path for a specific persona."
  (merge-pathnames
   (make-pathname :name "IMMEDIATE-GOALS" :type "md")
   (persona-directory (get-name persona-config))))

(defun read-persona-long-term-goals (persona-config)
  "Reads the LONG-TERM-GOALS.md file contents for a specific persona as a string, or returns NIL if it does not exist."
  (let ((path (persona-long-term-goals-file persona-config)))
    (and (probe-file path)
         (uiop:read-file-string path))))

(defun read-persona-short-term-goals (persona-config)
  "Reads the SHORT-TERM-GOALS.md file contents for a specific persona as a string, or returns NIL if it does not exist."
  (let ((path (persona-short-term-goals-file persona-config)))
    (and (probe-file path)
         (uiop:read-file-string path))))

(defun read-persona-immediate-goals (persona-config)
  "Reads the IMMEDIATE-GOALS.md file contents for a specific persona as a string, or returns NIL if it does not exist."
  (let ((path (persona-immediate-goals-file persona-config)))
    (and (probe-file path)
         (uiop:read-file-string path))))

(defun write-persona-long-term-goals (persona-config content)
  "Writes the LONG-TERM-GOALS.md file contents for a specific persona."
  (let ((path (persona-long-term-goals-file persona-config)))
...
(defun write-persona-short-term-goals (persona-config content)
  "Writes the SHORT-TERM-GOALS.md file contents for a specific persona."
  (let ((path (persona-short-term-goals-file persona-config)))
    (ensure-directories-exist path)
    (with-open-file (stream path :direction :output
                                 :if-does-not-exist :create
...
(defun write-persona-immediate-goals (persona-config content)
  "Writes the IMMEDIATE-GOALS.md file contents for a specific persona."
  (let ((path (persona-immediate-goals-file persona-config)))
    (ensure-directories-exist path)
    (with-open-file (stream path :direction :output
                                 :if-does-not-exist :create
...
(defun persona-goals-tools (persona-config)
  "Returns a list of tool objects for reading and writing persona goal files."
  (list
   ;; write-long-term-goals
   (cons (function-declaration
          :name "writeLongTermGoals"
          :description "Overwrites the LONG-TERM-GOALS.md file contents for the persona with the new goals markdown content."
          :behavior :blocking
          :parameters (schema
                       :type :object
                       :properties (object
                                    :content (schema :type :string
                                                     :description "The complete new markdown content for long term goals."))
                       :required (vector :content)))
         (lambda (&key content)
           (write-persona-long-term-goals persona-config content)
           "Long-term goals updated successfully."))

   ;; write-short-term-goals
   (cons (function-declaration
          :name "writeShortTermGoals"
          :description "Overwrites the SHORT-TERM-GOALS.md file contents for the persona with the new goals markdown content."
          :behavior :blocking
          :parameters (schema
                       :type :object
                       :properties (object
                                    :content (schema :type :string
                                                     :description "The complete new markdown content for short term goals."))
                       :required (vector :content)))
         (lambda (&key content)
           (write-persona-short-term-goals persona-config content)
           "Short-term goals updated successfully."))

   ;; write-immediate-goals
   (cons (function-declaration
          :name "writeImmediateGoals"
          :description "Overwrites the IMMEDIATE-GOALS.md file contents for the persona with the new goals markdown content."
          :behavior :blocking
          :parameters (schema
                       :type :object
                       :properties (object
                                    :content (schema :type :string
                                                     :description "The complete new markdown content for immediate goals."))
                       :required (vector :content)))
         (lambda (&key content)
           (write-persona-immediate-goals persona-config content)
           "Immediate goals updated successfully."))

   ;; read-long-term-goals
   (cons (function-declaration
          :name "readLongTermGoals"
          :description "Reads the LONG-TERM-GOALS.md file contents for the persona."
          :behavior :blocking
          :parameters (schema :type :object))
         (lambda ()
           (or (read-persona-long-term-goals persona-config)
               "No long-term goals found.")))

   ;; read-short-term-goals
   (cons (function-declaration
          :name "readShortTermGoals"
          :description "Reads the SHORT-TERM-GOALS.md file contents for the persona."
          :behavior :blocking
          :parameters (schema :type :object))
         (lambda ()
           (or (read-persona-short-term-goals persona-config)
               "No short-term goals found.")))

   ;; read-immediate-goals
   (cons (function-declaration
          :name "readImmediateGoals"
          :description "Reads the IMMEDIATE-GOALS.md file contents for the persona."
          :behavior :blocking
          :parameters (schema :type :object))
         (lambda ()
           (or (read-persona-immediate-goals persona-config)
               "No immediate goals found.")))
   ))
...
(eval-when (:compile-toplevel :load-toplevel :execute)
  ...
  (setf (documentation '*qwen* 'variable) "The content generator for Qwen.")
  )
...
(eval-when (:load-toplevel :execute)
  ...
  (setq *qwen* (persona-name->content-generator "Qwen3.6-27b"))
  )
