...
(defun thin-response-retry-enabled-p (content-generator)
  "Returns whether thin-response recursive retries should be used for CONTENT-GENERATOR.
LM Studio models can emit repeated zero-candidate responses that cause long requery loops,
so thin-response retries stay disabled there until a backend-specific strategy exists."
  (not (typep (get-backend content-generator) 'lmstudio-backend)))
...
(defun auto-personality-enabled-p (content-generator)
  "Returns true when the rotating personality prompt should be injected automatically."
  (and *enable-personality*
       (not (eq :lmstudio-api (get-googleapi content-generator)))))
...
(defun compute-system-instruction-contents (content-generator mood system-instruction)
  ...
  (append
   (or (and system-instruction
       ...
       (when (auto-personality-enabled-p content-generator)
...
(defun build-gemini-payload (content-generator context mood effective-prompt effective-turbo parts files system-instruction
                            &key (tools nil tools-supplied-p)
  ...
  (let* ((file-parts (when files (prepare-file-parts files)))
         ...
         (auto-tools-enabled-p (or (not (typep (get-backend content-generator) 'lmstudio-backend))
                                   (get-enable-lmstudio-tools content-generator)))
         (auto-tools (and auto-tools-enabled-p
                          (get-tools content-generator)))
         (effective-tools (if tools-supplied-p
                              tools
                              auto-tools))
         (effective-tool-config (if tool-config-supplied-p
                                    tool-config
                                    (and effective-tools
    ...
    (when effective-tools
      (setf (get-tools payload) effective-tools))
    (when effective-tool-config
      (setf (get-tool-config payload) effective-tool-config))
...
(defun generate-content (content-generator context mood prompt parts files system-instruction
                          &key (tools nil tools-supplied-p)
                               (tool-config nil tool-config-supplied-p)
...
(defun %generate-content (content-generator context mood prompt parts files system-instruction turbo depth
                         &key (tools nil tools-supplied-p)
                              (tool-config nil tool-config-supplied-p)
                              (read-timeout 300) (connect-timeout 60))
  ...
  (multiple-value-bind (payload prompt-contents-and-context)
      (apply #'build-gemini-payload
             content-generator context mood prompt turbo parts files system-instruction
             (append
              (when tools-supplied-p
                (list :tools tools))
              (when tool-config-supplied-p
    ...
    (let ((current-model (or (and turbo
      ...
      (restart-case
            ...
            (let* ((candidates (get-candidates response*))
                   (first-candidate (typecase candidates
                                      ...
                                      (vector (when (plusp (length candidates))
                                                (aref candidates 0))))))
              (let ((thin-response-retry-enabled-p
                     (thin-response-retry-enabled-p content-generator)))
                (when (and usage-metadata
                           thin-response-retry-enabled-p
                           (> (or (get-thoughts-token-count usage-metadata) 0) 10)
                ...
                (when (and usage-metadata
                           thin-response-retry-enabled-p
                           (not (> (or (get-candidates-token-count usage-metadata) 0) 0)))
        ...
        (use-weaker-model ()
          ...
          (apply #'%generate-content
                 content-generator context mood prompt parts files system-instruction nil (1+ depth)
                 (append
                  (when tools-supplied-p
                    (list :tools tools))
                  (when tool-config-supplied-p
