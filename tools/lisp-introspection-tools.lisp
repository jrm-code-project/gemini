      ...
      (let* ((narrow-string (str:trim string))
        ...
        (if (or (eq *enable-eval* :yolo)
            ...
            (let ((form (read-full-forms narrow-string)))
              (if form
                  (progn
                   (log-debug "Evaluating: ~s" form)
                    (let ((values (multiple-value-list (eval form))))
                     (cond ((null values) (log-debug "No values"))
                           ((= (length values) 1)
                            (log-debug "Value: ~S" (car values)))
                           (t (log-debug "Values:~%~{;;   * ~S~%~}" values)))
                     (values-list values)))
                  (progn
                   (log-debug "Incomplete expression: ~s" narrow-string)
        ...
        (let ((form (read-full-forms narrow-string)))
          (if form
              (let ((expanded (multiple-value-list (macroexpand form))))
                (log-debug "Macroexpand values:~%~{~s~^~%~}" expanded)
                expanded)
              (progn
                (log-debug "Incomplete expression: ~s" narrow-string)
                (error "End of expression reached early.  Check your expression.  Parentheses **must** be balanced.")))))))
        ...
        (let ((form (read-full-forms narrow-string)))
          (if form
              (let ((expanded (multiple-value-list (macroexpand-1 form))))
                (log-debug "Macroexpand-1 values:~%~{~s~^~%~}" expanded)
                expanded)
              (progn
                (log-debug "Incomplete expression: ~s" narrow-string)
                (error "End of expression reached early.  Check your expression.  Parentheses **must** be balanced.")))))))
