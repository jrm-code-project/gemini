   ...
   (list
    ...
    (cons
     ...
     (lambda (&key pathname (mime-type "application/octet-stream"))
       (log-debug "Reading binary file: ~a" (handle-tilde pathname))
       (object
    ...
    (cons
     (function-declaration
      ...
      :response (schema :type :array
                        :items (schema :type :string)))
     (lambda (&key pathname)
       (log-debug "Reading file: ~a" (handle-tilde pathname))
       (collect 'vector
    ...
    (cons
     ...
     (lambda (&key file blob)
       (let ((file* (handle-tilde file)))
         ...
         (log-debug "Directories exist: ~a" file*)
         (backup-file file*)
         (log-info "Writing blob to file: ~a" file*)
    ...
    (cons
     ...
     (lambda (&key file lines)
       (let ((file* (handle-tilde file)))
         ...
         (log-debug "Directories exist: ~a" file*)
         (backup-file file*)
         (log-info "Writing ~d lines to file: ~a" (length lines) file*)
